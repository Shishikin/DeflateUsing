...
local int been_here(int syms, int left, int len, int mem, int rem) {
    ...
    if (length <= offset) {
        ...
        if (length) {
            ...
            vector = (char*)(realloc(g.done[index].vec, length));
...
int main(int argc, char **argv) {
    ...
    g.code = (int*)(calloc(g.max + 1, sizeof(int)));
    ...
    if (syms == 2)              // iff max == 1
        ...
        assert(g.size <= (size_t)-1 / n && "overflow");
        g.size *= n;
        g.num = (big_t*)(calloc(g.size, sizeof(big_t)));
    ...
    printf("%d total codes for 2 to %d symbols", sum, syms);
    if (g.max < syms - 1)
    ...
    if (syms == 2)
        ...
        g.done = (tab*)(calloc(g.size, sizeof(struct tab)));
